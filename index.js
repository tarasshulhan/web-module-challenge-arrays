...
function filterByWord(arr, str){
  const temp = [];
  arr.forEach((item) => item.includes(str) && temp.push(item));
  return temp;
}
...
function getRandomFlavors(arr1, arr2, arr3, arr4){
  const allArrs = arr1.concat(arr2, arr3, arr4);
  let temp = [];
  while(temp.length < 31){
    temp.push(allArrs[Math.floor(Math.random()*allArrs.length)])
  }
  return temp;
}
let globTemp = getRandomFlavors(originalFlavors, newFlavors, seasonalFlavors, regionalFlavors);
console.log(globTemp, ' - ', globTemp.length);
